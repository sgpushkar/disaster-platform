...
export default function Dashboard() {
  ...
  const lineChartData = {
    ...
    datasets: [{
      ...
      pointBackgroundColor: historyForChart.map(h =>
        RISK_COLORS[h.risk_level] || '#ef4444'
      ),
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        backgroundColor: '#111114', titleColor: '#fafafa', bodyColor: '#ef4444',
        borderColor: '#27272a', borderWidth: 1, padding: 10, cornerRadius: 8,
        callbacks: { label: ctx => `Estimated Risk: ${ctx.parsed.y}/100` }
      }
    },
    scales: {
      x: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#71717a', font: { size: 11 } } },
      y: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#71717a', font: { size: 11 } }, min: 0, max: 100 },
    },
  }
  ...
  const barChartData = {
    ...
    datasets: [{
      ...
      backgroundColor: ['rgba(239,68,68,0.85)', 'rgba(220,38,38,0.7)', 'rgba(185,28,28,0.55)', 'rgba(153,27,27,0.4)'],
  ...
  const barOptions = {
    ...
    scales: {
      ...
      y: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#71717a', font: { size: 11 } } },
    },
  }
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs font-mono text-zinc-500 tracking-wider">ACQUIRING DISASTER INTEL TELEMETRY...</p>
      </div>
    )
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            {data?.active_warnings_count > 0 && (
              <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-red-500/15 text-red-400 border border-red-500/30 animate-pulse">
                {data.active_warnings_count} ACTIVE WARNING{data.active_warnings_count > 1 ? 'S' : ''}
              </span>
            )}
          </div>
          <p className="text-xs text-zinc-400 mt-1">Multi-signal environmental early warning & rapid evacuation dispatch</p>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          >
            <RefreshCw className={`h-3.5 w-3.5 ${refreshing ? 'animate-spin text-red-500' : ''}`} />
      ...
      {error && (
        <div className="card-panel p-3.5 border-red-500/30 bg-red-500/5 flex items-center gap-3 text-xs text-red-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        >
          <div className="w-full flex items-center justify-between pb-2 border-b border-slate-800">
            <span className="text-xs font-bold font-mono uppercase text-zinc-300 flex items-center gap-1.5">
              <Gauge className="h-3.5 w-3.5 text-red-500" />
              Composite Flood Hazard
            </span>
            <span className="text-[10px] font-mono text-zinc-500">{locationName || 'Sector pending'}</span>
          </div>
          <RiskGauge
          ...
          />
          {currentRec && (
            <p className="text-[11px] text-zinc-400 text-center leading-relaxed max-w-xs font-sans">
        ...
        >
          <div className="flex items-center justify-between pb-2 border-b border-slate-800">
            <span className="text-xs font-bold font-mono uppercase text-zinc-300 flex items-center gap-1.5">
              <Droplets className="h-3.5 w-3.5 text-zinc-400" />
              Sensor Telemetry
            </span>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { label: 'Temperature', value: data?.current_weather?.temperature != null ? `${data.current_weather.temperature.toFixed(1)}°C` : '—', icon: Thermometer, color: 'text-red-400' },
              { label: 'Humidity', value: data?.current_weather?.humidity != null ? `${data.current_weather.humidity}%` : '—', icon: Droplets, color: 'text-zinc-300' },
              ...
              { label: 'Wind Speed', value: data?.current_weather?.wind_speed != null ? `${data.current_weather.wind_speed} m/s` : '—', icon: Wind, color: 'text-zinc-400' },
              { label: 'Pressure', value: data?.current_weather?.pressure != null ? `${data.current_weather.pressure} hPa` : '—', icon: Activity, color: 'text-zinc-400' },
              { label: 'Sector', value: data?.current_weather?.location_name || locationName || '—', icon: MapPin, color: 'text-emerald-400' },
            ].map(({ label, value, icon: Icon, color }) => (
              <div key={label} className="p-2.5 rounded-lg bg-[#0e0e11] border border-slate-800">
                <div className="flex items-center gap-1 mb-1">
                  <Icon className={`h-3 w-3 ${color}`} />
                  <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-wider">{label}</span>
                </div>
                <p className="text-sm font-bold font-mono text-zinc-100 truncate">{value}</p>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        >
                ...
                {currentFactors.map((f) => (
                  <div key={f.key} className="space-y-1">
                    <div className="flex justify-between text-[11px] font-mono">
                      ...
                      <span className="text-red-400 font-semibold">{f.delta}</span>
                    </div>
          ...
          <div className="card-panel p-4 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              ...
              <Link to="/alerts" className="text-[11px] text-red-400 hover:text-red-300 flex items-center gap-0.5 font-mono">
            ...
            <div className="space-y-2">
              {data?.recent_alerts?.length > 0 ? (
                data.recent_alerts.slice(0, 3).map((alert) => (
                  <div key={alert.id} className="p-2.5 rounded-lg bg-[#0e0e11] border border-slate-800 text-xs space-y-1">
                    <div className="flex items-center justify-between gap-2">
                      <span className={`px-1.5 py-0.5 rounded text-[10px] font-mono font-bold uppercase ${
                        alert.risk_level === 'Critical' ? 'text-red-400 bg-red-500/10 border border-red-500/20' :
                ...
                <p className="text-xs text-zinc-500 py-2 text-center font-mono">No active alerts logged.</p>
              )}
            </div>
          </div>
        </motion.div>
      ...
      >
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-800">
          <div>
            <h2 className="text-sm font-bold font-mono uppercase text-zinc-200 flex items-center gap-1.5">
          ...
          <Link to="/predict" className="text-[11px] text-red-400 hover:text-red-300 font-mono">
      ...
      >
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-800">
          ...
          <span className="text-[10px] font-mono text-zinc-500">0 = Minimum · 100 = Critical Alert</span>
        </div>
        <div className="h-60">
          {riskHistory.length > 0 ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center border border-dashed border-slate-800 rounded-lg p-6">
              ...
              <p className="text-sm text-zinc-400 font-medium">No snapshots logged yet</p>
        ...
        {riskHistory.length > 0 && (
          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-800 flex-wrap">
            {Object.entries(RISK_COLORS).map(([level, color]) => (
              <div key={level} className="flex items-center gap-1.5 text-[11px] font-mono text-zinc-400">
      ...
      >
        {[
          ...
          { to: '/map', icon: Waves, label: 'GIS Tactical Map', desc: 'Danger zone perimeters & road routes', color: 'text-red-400', bg: 'bg-red-500/10 border-red-500/25 hover:border-red-500/50' },
          { to: '/alerts', icon: Bell, label: 'Incident Advisory Log', desc: `${data?.active_warnings_count ?? 0} active · tap to inspect`, color: 'text-red-400', bg: 'bg-red-500/10 border-red-500/25 hover:border-red-500/50' },
